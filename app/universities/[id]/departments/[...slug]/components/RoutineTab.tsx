...
import { api, Routine, getFullImageUrl, getApiKey, getApiUrl } from "@/lib/api"
import { EmptyState, ConfirmDelete, Modal, Field, inputCls } from "./SharedUI"
import { Calendar, Plus, Edit2, Trash2, Save, AlertCircle, Clock, Upload, X, Loader2 } from "lucide-react"
...
function RoutineModal({ open, onClose, routine, universityId, departmentId, onSuccess }: RoutineModalProps) {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState("")

  const [title, setTitle] = useState(routine?.title || "")
  const [imageFile, setImageFile] = useState<File | null>(null)
  const [imagePreview, setImagePreview] = useState<string | null>(routine?.image_url ? getFullImageUrl(routine.image_url) : null)
  const [time, setTime] = useState(routine?.time || "")

  // Reset form when opened with new routine
  useEffect(() => {
    if (open) {
      setTitle(routine?.title || "")
      setImageFile(null)
      setImagePreview(routine?.image_url ? getFullImageUrl(routine.image_url) : null)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!imagePreview && !imageFile) return setError("Routine image is required")

    setLoading(true)
    setError("")

    try {
      let image_url = routine?.image_url || ""

      if (imageFile) {
        // 1. Delete old image if exists
        if (routine?.image_url) {
          try {
            await fetch(`${getApiUrl()}/upload?url=${routine.image_url}`, {
              method: 'DELETE',
        ...
        uploadFormData.append("folder", "routines")
      ...
      if (routine) {
        await api.routines.update(routine.id, payload)
      } else {
  ...
  return (
    <Modal open={open} onClose={onClose} title={routine ? "Edit Routine" : "Add Routine"} className="max-w-md">
      <form onSubmit={handleSubmit} className="flex flex-col h-full">
        ...
        <div className="px-6 py-4 space-y-4">
          <Field label="Title" required>
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} className={inputCls} placeholder="e.g. Spring 2024 Class Routine" required />
          </Field>
          
          <Field label="Routine Image" required>
        ...
        <div className="border-t bg-muted/30 p-4 px-6 flex justify-end gap-2 mt-auto">
          ...
          <button type="submit" disabled={loading} className="px-4 py-2 text-sm font-medium rounded-sm bg-primary text-primary-foreground hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center gap-2 shadow-sm">
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} {loading ? "Saving..." : "Save Routine"}
          </button>
        </div>
      </form>
    </Modal>
...
export function RoutineTab({
  ...
  async function handleDelete() {
    ...
    setLoading(true)
    try {
      await api.routines.delete(selected.id)
      onRefresh()
      setDeleteOpen(false)
    } catch (e) {
      console.error(e)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold">Class Routines</h3>
          <p className="text-xs text-muted-foreground">Manage class and exam routines for the department.</p>
        </div>
        <button
      ...
      {!routines.length ? (
        <EmptyState icon={Calendar} label="routines" />
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {routines.map((routine) => (
            <div key={routine.id} className="group relative rounded-sm border bg-card shadow-sm hover:shadow-md transition-all flex flex-col">
              <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <button
                  onClick={() => { setSelected(routine); setModalOpen(true) }}
                  className="p-1.5 bg-background border rounded-sm hover:bg-muted text-primary transition-colors"
                >
                  <Edit2 className="h-3.5 w-3.5" />
                </button>
                <button
                  onClick={() => { setSelected(routine); setDeleteOpen(true) }}
              ...
              <div className="aspect-[4/3] bg-muted relative border-b overflow-hidden rounded-t-sm">
                {routine.image_url ? (
                  <img 
                    src={getFullImageUrl(routine.image_url)} 
                    alt={routine.title} 
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center bg-muted text-muted-foreground/40 gap-2">
                    ...
                    <span className="text-xs font-medium">No routine image</span>
                  </div>
                )}
              </div>

              <div className="p-4 flex flex-col flex-1">
                <h4 className="font-bold text-sm mb-1 truncate pr-16">{routine.title}</h4>
                {routine.time && (
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <Clock className="h-3.5 w-3.5" />
                    <span className="truncate">{routine.time}</span>
                  </div>
                )}
              </div>
            </div>
...
function RoutineTabModal(props: RoutineModalProps) {
  // Little wrapper just to ensure clean unmount if we need to reset state
  if (!props.open) return null
  return <RoutineModal {...props} />
}
