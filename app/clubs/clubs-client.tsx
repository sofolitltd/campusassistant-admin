...
import { api, Club, getFullImageUrl } from "@/lib/api"
...
export default function ClubsClient({ initialClubs }: ClubsClientProps) {
  ...
  const pendingCount = clubs.filter(c => !c.is_active).length
  ...
  async function handleDelete() {
    ...
    try {
      await api.clubs.delete(deleteId)
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Clubs &amp; Organizations</h1>
          <p className="text-muted-foreground text-sm">Manage the campus club directory shown in the app.</p>
        </div>
        <Link
          href="/clubs/add"
      ...
      {visibleClubs.length === 0 ? (
        <div className="flex h-64 flex-col items-center justify-center rounded-xl border-2 border-dashed bg-card/50 text-center">
          ...
          <h3 className="text-base font-bold">No clubs found</h3>
          <p className="mt-1 max-w-xs text-xs text-muted-foreground font-medium">
            Create the first club or student organization.
          </p>
          <Link href="/clubs/add" className="mt-4 flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground shadow-sm">
            <Plus className="h-4 w-4" /> Create First Club
          </Link>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          ...
          >
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest">Add Club</span>
          </Link>
        </div>
      )}
