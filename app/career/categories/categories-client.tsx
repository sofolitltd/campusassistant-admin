...
export function CategoriesClient() {
  ...
  useEffect(() => {
    api.careerCircularCategories.getAll()
      .then(setCategories)
      .catch(console.error)
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.careerCircularCategories.delete(deleteId)
    ...
    <div className="space-y-6">
      ...
      {sorted.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
          ...
          <p className="text-sm">Create your first career circular category.</p>
          <Link
            href="/career/categories/add"
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {sorted.map(cat => {
            const Icon = (Icons as unknown as Record<string, React.ElementType>)[cat.icon_key] || Icons.Briefcase
            return (
              <div
                key={cat.id}
                  ...
                  <div className="relative">
                    ...
                    {menuOpen === cat.id && (
                      <div className="absolute right-0 top-10 z-10 w-36 rounded-md border bg-card shadow-lg">
                        <Link
                          href={`/career/categories/edit/${cat.id}`}
