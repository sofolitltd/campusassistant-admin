...
import { api, AssociationEvent } from "@/lib/api"
import { Field, inputCls } from "@/app/universities/[id]/departments/[...slug]/components/SharedUI"

interface AssociationEventManagerProps {
  associationId: string
}

export function AssociationEventManager({ associationId }: AssociationEventManagerProps) {
  ...
  useEffect(() => {
    api.associationEvents.getByAssociation(associationId)
      .then(setEvents)
      .catch(console.error)
  ...
  async function handleAdd() {
    ...
    if (publishNow && !confirm("This will immediately push a notification to everyone following this association. Continue?")) {
      return
    }
    setAdding(true)
    try {
      const created = await api.associationEvents.create({
  ...
  async function handleDelete(id: string) {
    ...
    setDeletingId(id)
    try {
      await api.associationEvents.delete(id)
      ...
      <div className="rounded-lg border border-dashed p-4 space-y-3 bg-muted/10">
        ...
        <label className="flex items-start gap-2 cursor-pointer rounded-md bg-amber-50 dark:bg-amber-900/10 p-2.5 border border-amber-200 dark:border-amber-900/30">
          ...
          <span className="text-xs">
            ...
            <span className="text-muted-foreground">Sends a one-time push notification to everyone following this association. Cannot be undone.</span>
          </span>
        </label>
