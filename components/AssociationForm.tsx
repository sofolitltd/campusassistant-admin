...
import { api, Association, AssociationType, ASSOCIATION_CATEGORIES, University, BDDistrict } from "@/lib/api"
...
export function AssociationForm({ initialData, returnUrl }: AssociationFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    association_type: (initialData?.association_type || "district") as AssociationType,
  ...
  useEffect(() => {
    ...
    api.bdDistricts.getAll().then(setDistricts).catch(console.error)
  }, [])
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (type === "checkbox") {
      ...
      setFormData(prev => ({ ...prev, district_id: value, sub_district_id: "" }))
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.association_type === "sub_district" && !formData.sub_district_id) {
      alert("Please select a sub-district for a sub-district-scoped association")
      return
    }
    ...
    try {
      ...
      if (logoFile) {
        ...
        logo_url = await uploadImage(logoFile, "associations")
      ...
      if (bannerFile) {
        ...
        banner_url = await uploadImage(bannerFile, "associations")
      } else if (bannerPreview === null) {
        banner_url = ""
      }

      const subDistrict = formData.association_type === "sub_district"
        ? selectedDistrict?.sub_districts.find(sd => sd.id === formData.sub_district_id)
        : undefined
      ...
      const payload: Partial<Association> = {
        ...
        sub_district_id: formData.association_type === "sub_district" ? formData.sub_district_id : null,
        sub_district_name: formData.association_type === "sub_district" ? (subDistrict?.name || "") : null,
      ...
      if (initialData?.id) {
        await api.associations.update(initialData.id, payload)
      } else {
        await api.associations.create(payload)
      }
          ...
          <div className="space-y-4">
            <Field label="Name" required>
              <input
                ...
                placeholder="e.g. Rangpur Students' Association"
                className={inputCls}
              />
            </Field>
            ...
            <Field label="Description">
              <textarea
                ...
                onChange={handleChange}
                rows={4}
                placeholder="What does this association do?"
                className={inputCls}
              />
            </Field>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Association Type" required>
                <select name="association_type" value={formData.association_type} onChange={handleChange} className={selectCls}>
                  <option value="district">District-wide</option>
                  <option value="sub_district">Sub-district (Upazila)</option>
                </select>
              </Field>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="District" required>
                <select name="district_id" value={formData.district_id} onChange={handleChange} className={selectCls}>
            ...
            {formData.association_type === "sub_district" && (
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Sub-district" required>
                  <select name="sub_district_id" value={formData.sub_district_id} onChange={handleChange} className={selectCls}>
                    <option value="">Select sub-district</option>
                    {selectedDistrict?.sub_districts.map(sd => (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Contact Email">
              <input
                ...
                onChange={handleChange}
                placeholder="association@university.edu"
                className={inputCls}
              />
            </Field>
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          >
            ...
            {initialData ? "Save Changes" : "Create Association"}
          </button>
        </div>
      </div>
    </form>
