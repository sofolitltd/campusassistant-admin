...
interface LostFoundCategoryFormProps {
  initialData?: LostFoundCategory
  returnUrl: string
  onSaved?: (category: LostFoundCategory) => void
}

// icon_key stores a lucide-react icon component name (e.g. "Key", "Wallet",
// "Backpack") so both admin and the Flutter app can render a consistent icon
// without needing an uploaded image per category.
export function LostFoundCategoryForm({ initialData, returnUrl, onSaved }: LostFoundCategoryFormProps) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [formData, setFormData] = useState({
    name: initialData?.name || "",
    icon_key: initialData?.icon_key || "Package",
  ...
  const PreviewIcon = (Icons as unknown as Record<string, React.ElementType>)[formData.icon_key] || Icons.Package

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    try {
      const payload: Partial<LostFoundCategory> = {
        name: formData.name,
        icon_key: formData.icon_key,
        index: parseInt(formData.index) || 0,
      }

      let saved: LostFoundCategory
      if (initialData?.id) {
        saved = await api.lostFoundCategories.update(initialData.id, payload)
      } else {
        saved = await api.lostFoundCategories.create(payload)
      }
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Keys & Access Cards"
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="Lucide icon name, e.g. Key, Wallet, Backpack"
                className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/20"
              />
